...
/**
 * Récupère les données analytiques et les services associés pour un service donné.
 * Cette fonction est mise en cache pour éviter les recalculs inutiles.
 */
const getServiceAnalytics = cache((serviceId: string) => {
  ...
  const totalReviews = 127; // TODO: Remplacer par de vraies données dynamiques
  const avgRating = 4.9;
...
/**
 * Vérifie l'existence d'une image dans le dossier public et retourne le chemin
 * ou un chemin de fallback. Utilise `existsSync` pour éviter les erreurs 404.
 */
const getVerifiedImage = (imagePath: string, fallback: string = '/images/services/default-service.jpg'): string => {
  try {
    ...
    console.error(`Erreur de vérification d'image pour le chemin ${imagePath}:`, error);
    return fallback;
  }
};
...
// ============================================================================
// 🔍 PARTIE 4: MÉTADONNÉES SEO EXPERT + DONNÉES STRUCTURÉES
// ============================================================================

/**
 * Génère les métadonnées SEO et les données structurées pour une page de service.
 * Gère les redirections des anciennes URLs.
 */
export async function generateMetadata({ params }: { params: { serviceId: string } }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        ...
        },
        {
          ...
          width: 800,
          height: 800,
          alt: `Icône ${service.id}`, // Utiliser l'id pour un alt unique
        }
      ],
      locale: 'fr_FR',
      type: 'website',
    ...
    alternates: {
      ...
      languages: {
        'fr-FR': canonical,
        // TODO: Ajouter les autres langues ici
      }
    },
    robots: {
      index: true,
...
/**
 * Génère les données structurées Schema.org pour un service spécifique.
 * Le format @graph permet de lier plusieurs entités (WebPage, Service, Organization...).
 */
const generateAdvancedStructuredData = (analytics: NonNullable<ReturnType<typeof getServiceAnalytics>>) => {
  ...
  const heroImage = getVerifiedImage(`/images/services/${service.id}-og.jpg`); // Utilisation de l'image OG qui est plus grande
...
const useConsentTracking = () => {
  // Simulez une logique de consentement simplifiée
  const hasConsent = typeof window !== 'undefined' && (window as any).hasMarketingConsent;
  return {
...
export default function ServiceDetailPage({ params }: { params: { serviceId: string } }) {
  ...
  // Redirection gérée directement dans generateMetadata, mais on peut la laisser ici aussi
  if (URL_REDIRECTS[serviceId]) {
    redirect(`/service/${URL_REDIRECTS[serviceId]}`);
  }
  ...
  if (!analytics) {
    notFound();
    // Le return null est redondant après notFound()
  }
  ...
  // Utilisation de l'image OG comme image principale
  const heroImage = getVerifiedImage(`/images/services/${service.id}-og.jpg`); 
  ...
  return (
    <>
      ...
      <article className="bg-cream min-h-screen" itemScope itemType="https://schema.org/Service">
        ...
        {/* Ajout de l'image pour les microdata */}
